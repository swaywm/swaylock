...
static void schedule_gif_frame(struct swaylock_state *state) {
  ...
  if (state->gif_timer != NULL) {
    return;
  }

  int delay = get_gif_min_delay(state);
  state->gif_timer =
      loop_add_timer(state->eventloop, delay, gif_timer_callback, state);
}
...
static void gif_timer_callback(void *data) {
  ...
  wl_list_for_each(surface, &state->surfaces, link) {
    if (surface->gif && surface->gif->is_animated) {
      ...
      cairo_surface_t *frame = gif_get_current_frame(surface->gif);

      if (!frame) {
		  swaylock_log(LOG_ERROR, "Failed to get current GIF frame for surface");
		  continue;
      }
...
static void load_image(char *arg, struct swaylock_state *state) {
  ...
  if (is_gif_path(image->path)) {
    ...
    if (image->gif) {
      ...
      swaylock_log(
          LOG_DEBUG, "Loaded GIF %s for output %s (%d frames, animated: %s)",
          image->path, image->output_name ? image->output_name : "*",
          image->gif->frame_count, image->gif->is_animated ? "yes" : "no");
      return;
    }
