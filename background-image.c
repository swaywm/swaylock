...
// These functions are deprecated but there's no replacement for animated image
// loading
...
struct swaylock_gif *load_gif_image(const char *path) {
  ...
  GdkPixbufAnimation *animation =
      gdk_pixbuf_animation_new_from_file(path, &err);
  if (!animation) {
    swaylock_log(LOG_ERROR, "Failed to load GIF animation (%s).", err->message);
    g_error_free(err);
    return NULL;
  }
  ...
  if (gdk_pixbuf_animation_is_static_image(animation)) {
    swaylock_log(LOG_DEBUG,
                 "GIF is static (single frame), loading as regular image");
  ...
  do {
    if (frame_count >= max_frames) {
      ...
      // Clean up already loaded frames before returning
    ...
    if (frame_count >= capacity) {
      ...
      cairo_surface_t **new_frames =
          realloc(frames, new_capacity * sizeof(cairo_surface_t *));
